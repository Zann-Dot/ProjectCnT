...
let listProductHTML = document.querySelector('.menu-container-box');
...
const addButtonToHTML = (product_id) => {
    ...
    if(listProducts.length > 0){
        ...
        newButton.classList.add('new-add-Button');
        listProducts.forEach(product => {   
            newButton.innerHTML = `
            <button class="minus">
...
const whenCartisEmpty = (product_id) => {
    ...
    const removeAddMinusButtons = document.querySelector('.new-add-Button');
    removeAddMinusButtons.style.display = 'none';
    const addFromHTML = document.querySelector(`.js-add-button-${product_id}`);
